...
def brainrenders(group,tau_mean):
    if 'dhcp' in group:
        ...
        atlas = nib.load('/dhcp/fmri_anna_graham/dhcp_camcan_timescales/data/schaefer_40weeks.nii.gz')
        outvolume_size = np.zeros((202, 274, 217))
    else:
        atlas  = nib.load('/dhcp/fmri_anna_graham/dhcp_camcan_timescales/data/Schaefer2018_400Parcels_17Networks_order_FSLMNI152_1mm_resliced.nii.gz')
        outvolume_size = np.zeros((61, 73, 61))

    atlas_data = atlas.get_fdata()

    ##### TAU distribution visualisation    
    print(f'Working on tau render for {group}....')
    render(range(0,tau_mean.shape[0]),tau_mean,atlas,outvolume_size,f"/dhcp/fmri_anna_graham/dhcp_camcan_timescales/results/{group}_tauvalues_render.nii.gz")

    ##### ROI visualisation and grouping

    ## Load ROI label file
    print(f'Working on roi renders....')
    network_file17 = pd.read_csv('/dhcp/fmri_anna_graham/dhcp_camcan_timescales/data/Schaefer2018_400Parcels_17Networks_order.txt',sep = '\t', header = None)
    ...
    with open('/dhcp/fmri_anna_graham/dhcp_camcan_timescales/data/roi_by_netclass.pickle','rb') as f:
    ...
    render(uni_vs_trans_index,roi_value,atlas, outvolume_size,f"/dhcp/fmri_anna_graham/dhcp_camcan_timescales/results/{group}_uni_vs_transmodal_render_all.nii.gz")
    ...
    render(net_idx,netnum_list,atlas,outvolume_size,f"/dhcp/fmri_anna_graham/dhcp_camcan_timescales/results/{group}_8networks_render.nii.gz")
